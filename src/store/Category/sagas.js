...
export function* getCategory(){
	try{
		...
		yield put({type:CATEGORY.GET_CATEGORY_DATA_LOADING})
		const response = yield call(setRestuarantDataAPI,"https://khajaorder.com/korderapi/category/view.php",tokenData);
		console.log('res from get',response);
		const data = response.data;
		yield put({type:CATEGORY.GET_CATEGORY_DATA_SUCCESS,data:data})
	}
	catch(err){
			yield put({type:CATEGORY.GET_CATEGORY_DATA_ERROR,error:err.message})
	}
...
export function* getCategoryData(){
	...
	yield takeLatest(CATEGORY.GET_CATEGORY_DATA_API,getCategory);
}
...
export default function* categoryData(){
	yield all([fork(saveCategory),fork(getCategoryData),fork(deleteCategory),fork(editCategory)]);
}